...
- **Persistence**: `@capacitor/preferences` via [usePersistedRef](src/composables/usePersistedRef.ts), wired in [useAppState.ts](src/composables/useAppState.ts). Two persisted refs: `preferredBroker` and `manualBrokers` (array). On startup, preferred broker loads as "Not found" → short (~5s) mDNS scan fills in IP/port. Manually-added brokers skip mDNS resolution (state = "manual", yellow bg, immediate auto-connect); they persist in `manualBrokers` and are listed on startup, de-duped on `host:port:type`. See [prefer-broker.md](prefer-broker.md) for the full evolution of this flow.
